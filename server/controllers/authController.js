...
const register = async (req, res) => {
    ...
    try {
        ...
        await connection.query(
            ...
            [userUuid, businessId, email, hashedPassword, first_name, last_name, 'admin']
        );

        await connection.commit();
        res.status(201).json({ message: 'Registration successful', business_uuid: businessUuid });
    } catch (error) {
...
const createDriver = async (req, res) => {
    const { business_id } = req.user;
    const { first_name, last_name, email, password, phone, vehicle_type, license_number } = req.body;
    ...
    try {
        ...
        const [userResult] = await connection.query(
            'INSERT INTO users (uuid, business_id, email, password_hash, first_name, last_name, user_type) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [userUuid, business_id, email, hashedPassword, first_name, last_name, 'driver']
        );
        ...
        await connection.commit();
        res.status(201).json({ message: 'Driver created successfully', uuid: userUuid });
    } catch (error) {
        await connection.rollback();
